...
export function ResumeViewer({ lang }: { lang: 'en' | 'th' }) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-fd-background text-fd-foreground rounded-2xl shadow-sm border border-fd-border mt-10">
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-fd-border pb-6 mb-6">
        ...
        <button className="mt-4 md:mt-0 flex items-center gap-2 px-4 py-2 bg-fd-primary text-fd-primary-foreground rounded-lg hover:bg-fd-primary/90 transition-colors">
      ...
      {/* Tabs */}
      <div className="flex gap-4 mb-6 border-b border-fd-border pb-2 overflow-x-auto">
        <button
        ...
        {activeTab === 'skills' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {resume.skills.map((skillGroup, idx) => (
              <div key={idx} className="bg-fd-card p-4 rounded-xl border border-fd-border">
                <h3 className="font-semibold text-lg mb-3 text-fd-card-foreground">{skillGroup.category}</h3>
                <div className="flex flex-wrap gap-2">
                  {skillGroup.items.map((item, i) => (
                    <span key={i} className="px-3 py-1 bg-fd-muted text-sm rounded-full border border-fd-border">
                      {item}
                    </span>
                  ))}
                </div>
        ...
        {activeTab === 'experience' && (
          <div className="space-y-6">
            {resume.experience.map((exp, idx) => (
              <div key={idx} className="relative pl-6 border-l-2 border-fd-muted">
                <div className="absolute w-3 h-3 bg-fd-primary rounded-full -left-[7px] top-2" />
                <h3 className="font-semibold text-lg">{exp.title}</h3>
                <p className="text-fd-primary font-medium">{exp.company}</p>
                <p className="text-sm text-fd-muted-foreground mb-3">{exp.period}</p>
                <ul className="space-y-2">
                  {exp.achievements.map((ach, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm">
                      <ChevronRight className="w-4 h-4 text-fd-primary shrink-0 mt-0.5" />
                      <span>{ach}</span>
                    </li>
                  ))}
                </ul>
        ...
        {activeTab === 'projects' && (
          <div className="grid grid-cols-1 gap-6">
            {resume.projects.map((proj, idx) => (
              <div key={idx} className="bg-fd-card p-5 rounded-xl border border-fd-border hover:border-fd-primary transition-colors">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-semibold text-lg text-fd-card-foreground">{proj.title}</h3>
                  <a href={proj.repoUrl} target="_blank" rel="noreferrer" className="text-xs text-fd-primary hover:underline">
                    View Repo
                  </a>
                </div>
                <ul className="space-y-2 mb-4">
                  {proj.description.map((desc, i) => (
                    <li key={i} className="text-sm flex items-start gap-2 text-fd-muted-foreground">
                      <span className="w-1.5 h-1.5 bg-fd-muted-foreground rounded-full shrink-0 mt-1.5" />
                      <span>{desc}</span>
                    </li>
                  ))}
                </ul>
